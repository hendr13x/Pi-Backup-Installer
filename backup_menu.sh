  ...
  while true; do
    ...
    case $opt in
      1)
        ...
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config NAS_IP '$v'"
        ;;
      2)
        read -rp "New Share: " v
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config NAS_SHARE '$v'"
        ;;
      3)
        read -rp "New Username: " v
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_cred username '$v'"
        ;;
      4)
        read -rp "New Password: " v
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_cred password '$v'"
        ;;
      5)
        read -rp "Max backups to keep (1–50): " v
        if [[ "$v" =~ ^[0-9]+$ ]] && (( v >= 1 && v <= 50 )); then
          sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config MAX_BACKUPS '$v'"
        else
      ...
      6)
        ...
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config AUTO_BACKUP_ENABLED '$en'"
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config AUTO_BACKUP_SCHEDULE '$sch'"
        ;;
      7)
        read -rp "Backup filename prefix: " v
        sudo bash -c "'$INSTALL_DIR/write_config.sh' set_config BACKUP_BASENAME '$v'"
        ;;
      q|Q)
        return
        ;;
